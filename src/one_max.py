...
def selBest(individuals, k):
    ...
    """
    # 1. Extraction vectorisée des fitnesses (suppose 1 seul objectif)
    ...
    # 2. ArgSort est beaucoup plus rapide qu'un sort() python sur des objets complexes
...
def run_steady_state(pop_size, max_iterations, selection_op, crossover_op, mutation_op, insertion_strategy, pc=1.0, pm=1.0):
    ...
    for it in range(1, max_iterations + 1):
        ...
        if insertion_strategy == "fitness":
            # OPTIMISATION NUMPY : Pas de sort() complet !
            # 1. On extrait les fitness actuelles dans un tableau numpy
            # (Note: sur de très grosses pop, maintenir ce tableau à jour serait encore mieux,
            # mais ici l'extraction est rapide comparée au tri)
            current_fits = np.array([ind.fitness.values[0] for ind in population])

            # 2. np.argpartition met les k plus petits éléments au début (non triés)
            ...
            # OPTIMISATION : Buffer Circulaire (Round-Robin)
            # Complexité O(1) vs O(N) pour pop(0)
        ...
        # --- LOGGING ---
        # Petite astuce : pour éviter selBest (qui trie tout) juste pour le log,
        # on peut garder le meilleur en mémoire, mais selBest(pop, 1) reste rapide sur petite pop.
        # Sur N=1000, max() sur array est mieux.
